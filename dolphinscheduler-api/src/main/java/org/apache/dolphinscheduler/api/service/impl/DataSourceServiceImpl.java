    ...
    public List<ParamsOptions> getTableColumns(User loginUser, Integer datasourceId, String database,
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int decDigits = rs.getInt(DECIMAL_DIGITS); // 小数位
                ...
                if (typeName != null && !typeName.isEmpty()) {
                    ...
                    // 哪些类型需要带精度(scale)
                    boolean needsSize = upper.contains("CHAR")
                            || upper.equals("NUMBER")
                            || upper.equals("DECIMAL")
                            || upper.equals("NUMERIC")
                            || upper.equals("DEC")
                            || upper.equals("DECFLOAT");
                    if (colSize > 0 && needsSize) {
                        // NUMBER(18, 4) 这种带小数位的要拼 (p, s)
                        // Oracle 的 NUMBER 不带精度时 colSize 是 0 或者 38 (NUMBER_MAX_PRECISION)
                        if (decDigits > 0) {
                            sb.append('(').append(colSize).append(',').append(decDigits).append(')');
                        } else {
                            sb.append('(').append(colSize).append(')');
                        }
                    } else if (upper.equals("NUMBER") && colSize == 0) {
                        // Oracle NUMBER 没有显式精度 = 可变精度, 显式声明 NUMBER(38,0) 让 Flink 接收
                        sb.append("(38,0)");
                    }
                }
